        ...
        <%
            ...
            if (request.getMethod().equalsIgnoreCase("POST")) {
                ...
                if ("update".equals(action)) {
                    ...
                    try {
                        String updateCalonMahasiswa = "UPDATE calon_mahasiswa SET status = ? WHERE id = ?";
                        try (PreparedStatement ps = db.getConnection().prepareStatement(updateCalonMahasiswa)) {
                            ps.setBoolean(1, status);
                            ps.setInt(2, calonMahasiswaId);
                            ps.executeUpdate();
                        }
            ...
            String query = "SELECT cm.id, cm.nama, cm.NISN, cm.status, pj.piljurusan1, pj.piljurusan2, pj.piljurusan3, pj.piljurusan4 " +
                        ...
                        <form method="post">
                            ...
                            <td>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="status" value="true" <%= rs.getBoolean("status") ? "checked" : "" %>>
                                    <label class="form-check-label">Lolos</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="status" value="false" <%= !rs.getBoolean("status") ? "checked" : "" %>>
